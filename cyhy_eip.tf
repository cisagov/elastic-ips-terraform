...
resource "aws_eip" "cyhy_us_east_1" {
  ...
  tags = merge(
    var.tags,
    {
      "Name" = format("CyHy EIP us-east-1_%d", count.index + 1)
    },
  )
...
resource "aws_eip" "cyhy_us_east_2" {
  ...
  tags = merge(
    var.tags,
    {
      "Name" = format("CyHy EIP us-east-2_%d", count.index + 1)
    },
  )
...
resource "aws_eip" "cyhy_us_west_1" {
  ...
  tags = merge(
    var.tags,
    {
      "Name" = format("CyHy EIP us-west-1_%d", count.index + 1)
    },
  )
...
resource "aws_eip" "cyhy_us_west_2" {
  ...
  tags = merge(
    var.tags,
    {
      "Name" = format("CyHy EIP us-west-2_%d", count.index + 1)
    },
  )
